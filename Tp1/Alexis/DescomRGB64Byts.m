...
function [BloqueRGB, TamBloqueAum, ...
    FlagSizeInadeCol, ...
    FlagSizeInadeFil] = DescomRGB64Byts(Imagen)

%%% Creamos las matrices que van a contener los datos de los bloques en los
%%% que se va a dividir la imagen. Como se vaa dividir la imagen en bloques
...
if FlagSizeInadeCol == 0
    if FlagSizeInadeFil == 0
        BloqueRGB = zeros(64,fil/8*col/8*prof);
    ...
    for j = 1:FilAmpl
        for i = 1:ColAmpl
            for p = 1:8 
                ...
                BloqueRGB(aux1:p*8,aux2) = ....
                    NuevImagen((j-1)*8+1:j*8,(i-1)*8+p,k);
