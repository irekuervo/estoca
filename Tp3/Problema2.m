...
EstimacionError E = zeros(20000,1);
EstimacionG  = zeros(L,1);
...
for i = 1:500
    ...
    EstimacionS = X-EstimacionG;
    
    EstimacionErrorE = EstimacionErrorE + (EstimacionS - S).^2/N;
    EstimacionErrorJ = EstimacionErrorJ + (EstimacionG - G).^2/N;    
end
...
for j = 1:5
    for i = 1:500
        ...
        for i = 1:L
            Error = X(i)-Entrada(i,:)*W(:,i);
            W(:,i+1) = W(:,i) + mu(j)*Entrada(i,:)'*Error;
            EstimacionG(i) =  Entrada(i,:)*W(:,i+1);
        end
